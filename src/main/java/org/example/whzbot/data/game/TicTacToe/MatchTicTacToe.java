...
public class MatchTicTacToe implements IMatch<TicTacToe> {
    TicTacToe board;
    Rule rule = new Rule();
    long[] players;
    boolean flip_order = false;
    int game_state = 0; // 0: waiting, 1:running, 2:ended.
    UUID uuid;
    ...
    public boolean move(String input) {
        if (this.game_state != 1 || !this.board.canMove(input))
            return false;
        this.board = this.board.doMove(input);
        if (this.board.isEnd())
            this.game_state = 2;
        return true;
    }
